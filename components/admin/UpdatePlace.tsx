...
const UpdatePlace = ({ data }: Props) => {
  const place = data?.place;

  const [placeDetails, setPlaceDetails] = useState({
    name: place?.name,
    description: place?.description,
    pageLink: place?.pageLink,
    address: place?.address,
    regularPrice: place?.price?.regular,
    reducedPrice: place?.price?.reduced,
    notes: place?.price?.notes,
  ...
  const [updatePlace, { isLoading, error, isSuccess }] =
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    updatePlace({ body: placeData, id: place._id });
  };
  ...
  return (
    <div className="row wrapper">
      <div className="col-10 col-lg-8">
        ...
        >
          <h2 className="mb-4">Update Place</h2>
          <div className="mb-3">
