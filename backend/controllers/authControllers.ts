...
export const uploadAvatar = catchAsyncErrors(async (req: NextRequest) => {
  ...
  const avatarResponse = await upload_file(body?.avatar, "places/avatars");
...
export const forgotPassword = catchAsyncErrors(async (req: NextRequest) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "Fun Places Password Recovery",
      message,
    });
  } catch (error: any) {
...
export const resetPassword = catchAsyncErrors(
  async (req: NextRequest, { params }: { params: { token: string } }) => {
    ...
    if (body.password !== body.confirmPassword) {
      throw new ErrorHandler("Passwords does not match", 404);
    }
...
export const allUsers = catchAsyncErrors(async (req: NextRequest) => {
